    ...
    public static class Utilities
    {
        ...
        public static void showMenuTeleNpc()
        {
            ...
            new MenuBuilder()
                ...
                {
                    string name = npc.template.name.Remove(npc.template.name.IndexOf('[') - 1);
                    var npcName = string.IsNullOrEmpty(name.Trim()) ? "(no name)" : name.Trim();
        ...
        {
            var index = getIndexItemBag(921, 454, 2074, 2075, 2105);
            if (index == -1)
            {
        ...
        [HotkeyCommand('u')]
        public static void getBlackFlag()
        {
        ...
        [ChatCommand("abf")]
        public static void setBuff(string text)
        {
            string[] arr = text.Split(',');
            AutoPet.hpBuff = int.Parse(arr[0]);
            AutoPet.mpBuff = int.Parse(arr[1]);
