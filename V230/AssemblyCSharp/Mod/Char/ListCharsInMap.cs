    ...
    public class ListCharsInMap
    {
        ...
        static void PaintRect(mGraphics g)
        {
            ...
            if (TileMap.mapID != Char.myCharz().cgender + 21)
            {
                ...
                string hexColor = $"#{(int)(color.r * 255):x2}{(int)(color.g * 255):x2}{(int)(color.b * 255):x2}{(int)(color.a * 255):x2}";
                str = $"<color=yellow>{TileMap.mapName}</color> khu <color=yellow>{TileMap.zoneID}</color> [<color={hexColor}>{listChars.Where(c => c.isNormalChar()).Count() + 1}</color>/<color=red>{GameScr.gI().maxPlayer[TileMap.zoneID]}</color>]";
            }
            else str = $"<color=yellow>{TileMap.mapName}</color>";
            GUIStyle style = new GUIStyle(GUI.skin.label)
            {
