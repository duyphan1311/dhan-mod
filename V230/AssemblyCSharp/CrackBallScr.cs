...
public class CrackBallScr : mScreen
{
	...
	public override void update()
	{
        ...
        {
            #region mod
            if (Mod.Auto.AutoCrackBall.gI.IsActing)
            {
                if (Input.GetKey((KeyCode)112))
                {
                    Mod.Auto.AutoCrackBall.gI.toggle(false);
                    Mod.Auto.AutoCrackBall.isNotEnough = false;
                    Mod.Auto.AutoCrackBall.isAuto = false;
                    gI().doClickSkill(1);
                    gI().doClickSkill(1);
