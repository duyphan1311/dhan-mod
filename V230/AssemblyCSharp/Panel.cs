    ...
    private void paintInventory(mGraphics g)
    {
        ...
        for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
        {
            ...
            if (item != null)
            {
                ...
                {
                    if (num2 == it.id && it.type == item.template.type && it.name == item.template.name)
                    {
                    ...
                    else
					{
                        ...
                        else
                        {
                            ...
                            if(HasItemInList)
                                ...
                                myVector.addElement(new Command("Dùng liên tục", AutoUseItem.gI, 1, currItem));
                            //foreach (AutoUseItem.ItemUse ita in AutoUseItem.listItemUse)
                            //{
                            //    if (currItem.template.id == ita.item.template.id && currItem.GetFullInfo() == ita.item.GetFullInfo())
                            //    {
                            //        myVector.addElement(new Command("Kết thúc", AutoUseItem.gI, 2, currItem));
                            //    }
                            //    else
