...
namespace Mod.Xmap
{
    public class XmapData
    {
        ...
        public XmapData()
        {
            ...
            loadLinksFromFile("Data\\XMapData\\LinkMapsXmap.txt");
            loadLinksAutoWaypointFromFile("Data\\XMapData\\AutoLinkMapsWaypoint.txt");
            addLinksHome();
            loadLinkSieuThi();
            loadLinkToCold();
        }
        ...
        public void loadLinkMapCapsule()
        {
            ...
            //    Pk9rXmap.isShowPanelMapTrans = false;
            //}
            if (Pk9rXmap.canUseCapsule() && !AutoGa.isAutoGa)
            {
                var index = Utilities.getIndexItemBag(Utilities.ID_ITEM_CAPSULE_VIP, Utilities.ID_ITEM_CAPSULE_NORMAL);
                LogMod.writeLine($"[xmap][dbg] Sử dụng {Char.myCharz().arrItemBag[index].template.name}");
                Service.gI().useItem(0, 1, index, -1);
                Pk9rXmap.isShowPanelMapTrans = false;
                //if (Utilities.hasItemCapsuleVip())
                //{
                //    LogMod.writeLine($"[xmap][dbg] Sử dụng capsule đặc biệt");
                //    Service.gI().useItem(0, 1, -1, Utilities.ID_ITEM_CAPSULE_VIP);
                //    Pk9rXmap.isShowPanelMapTrans = false;
                ...
                using (var reader = new StreamReader(path))
                {
                    ...
                    while ((textLine = reader.ReadLine()) != null)
                    {
                        ...
                        for (int j = 0; j < length; j++)
                        {
                            ...
                            {
                                if (data[j] >= 99) data[j] -= 3;
                                else if (data[j] >= 94) data[j] -= 2;
                                else if (data[j] >= 78) data[j]--;
                            }
                        }
