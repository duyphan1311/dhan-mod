...
namespace Mod.ModMenu
{
    public class ModMenuMain : IChatable
    {
        ...
        public static ModMenuItemBoolean[] modMenuItemBools = new ModMenuItemBoolean[]
        {
            ...
            new ModMenuItemBoolean("Hiện danh sách nhân vật", "Hiện danh sách nhân vật trong map", ListCharsInMap.setState, true, "isshowlistchar"),
            ...
            new ModMenuItemBoolean("Hiện khoảng cách bom", "Hiển thị người, quái, boss... trong tầm bom", SuicideRange.setState, true, "isshowsuiciderange"),
            new ModMenuItemBoolean("Nền tùy chỉnh", "Thay thế nền của game bằng nền tùy chỉnh (tự động điều chỉnh nền cho vừa kích thước màn hình)", CustomBackground.setState, false, "iscustombackground", true, "Bạn cần tắt chức năng \"Giảm đồ họa\"!"),
            ...
            new ModMenuItemBoolean("Thông báo Boss", "Bật/tắt hiển thị thông báo boss", Boss.setState, true, "sanboss"),
            ...
            new ModMenuItemBoolean("Check lag", "Bật/tắt Tự động đăng nhập lại khi lag", Utilities.setCheckLag, false, "", true, "Tính năng chỉ hoạt động khi tàn sát!!!"),
        ...
        {
            new ModMenuItemInt("FPS", null, "FPS mục tiêu (cần tắt Vsync để thay đổi có hiệu lực)", Application.targetFrameRate, delegate(int value)
            {
                if (value > 5 && value <= 144) Application.targetFrameRate = value;
        ...
        public static ModMenuItemFunction[] modMenuItemFunctions = new ModMenuItemFunction[]
        {
            ...
            new ModMenuItemFunction("Menu PickMob", "Mở menu PickMob (lệnh \"pm\")", Pk9rPickMob.ShowMenu),
            new ModMenuItemFunction("Menu Teleport", "Mở menu dịch chuyển (lệnh \"tele\" hoặc bấm nút q)", TeleportMenu.TeleportMenu.ShowMenu),
        ...
        public static Dictionary<string, int> defaultValueModMenuItemInts = new Dictionary<string, int>()
        {
        ...
        public static Dictionary<string, bool> defaultValueModMenuItemBools = new Dictionary<string, bool>()
        {
            ...
            { "isvuotdiahinh", true },
            { "isautopick", true },
            { "ispickmyitemonly", false },
            { "islimitpicktimes", false },
            { "infochar", true },
            { "infopet", true },
            { "isshowlistboss", true },
            { "isShowListItemAutoUse", true },
            { "isShareInfo", false },
            { "isGotoMapBoss", false }
        };
        ...
        {
            SetTabPanelTemplates.setTabListTemplate(panel, modMenuItemBools, modMenuItemInts, modMenuItemFunctions, ExtensionManager.Extensions, new List<string>());
            if (panel.currentTabIndex == 4) GuidePanel.setTabGuide(panel);
        ...
        public static void paintModMenu(Panel panel, mGraphics g)
        {
            if (panel.currentTabIndex == 0)
                ...
                GuidePanel.paintTabGuide(panel, g);
        }
        ...
        public static void onModMenuValueChanged()
        {
            ...
            modMenuItemBools[26].isDisabled = !Pk9rPickMob.IsTanSat;
        ...
        public static void LoadData()
        {
            foreach (ModMenuItemBoolean modMenuItem in modMenuItemBools)
            {
                try
                {
                    ...
                    else if (defaultValueModMenuItemBools.TryGetValue(modMenuItem.RMSName, out bool value))
                        modMenuItem.setValue(value);
                }
                catch { }
            }
            foreach (ModMenuItemInt modMenuItem in modMenuItemInts)
            {
                try
                {
                    ...
                    else if (defaultValueModMenuItemInts.TryGetValue(modMenuItem.RMSName, out int value))
                        modMenuItem.setValue(value);
            ...
            if (!string.IsNullOrEmpty(text))
            {
                if (to == inputModMenuItemInts[0][0])
                {
                    try
                    {
                        ...
                        if (value > 144 || value < 5) throw new Exception();
