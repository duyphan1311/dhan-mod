...
namespace Mod.Auto
{
    internal class AutoUseItem : IActionListener, IChatable
    {
        ...
        public static string CustomString(ItemUse itemUse)
        {
            ...
            if (itemUse.timeUse < 1000)
                strTimeUse += $"<color={timeDelay}>{itemUse.timeUse}</color>ms";
            else
            {
            ...
            if (timeSpan < 0)
                result += $"<color=orange>0</color>ms";
            else if (timeSpan < 1000)
                result += $"<color=orange>{timeSpan}</color>ms";
            else
            {
