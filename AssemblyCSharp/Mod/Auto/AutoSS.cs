...
namespace Mod.Auto
{
    public class AutoSS
    {
            ...
            else if (TileMap.mapID >= 21 && TileMap.mapID <= 23)
            {
                if (Char.myCharz().taskMaint.index == 2)
                {
                    if (Char.myCharz().cgender == 0)
                        Utilities.openMenu(0);
                    if (Char.myCharz().cgender == 1)
                        Utilities.openMenu(2);
                    if (Char.myCharz().cgender == 2)
                        Utilities.openMenu(1);
                }
        ...
        static ItemMap getItemNV3()
        {
            for (int i = GameScr.vItemMap.size() - 1; i >= 0; i--)
            {
                ItemMap itemMap = (ItemMap)GameScr.vItemMap.elementAt(i);
                if (itemMap.template.id == idDuaBe)
                    return itemMap;
            }
            return null;
        }
        ...
        static void AutoNV3()
        {
            ...
            if (Char.myCharz().taskMaint.index == 1)
            {
                ...
                {
                    if (Char.myCharz().cgender == 0)
                        Utilities.teleportMyChar(getItemNV3().x, getItemNV3().y);
                    if (Char.myCharz().cgender == 1)
                        Utilities.teleportMyChar(getItemNV3().x, getItemNV3().y);
                    if (Char.myCharz().cgender == 2)
                        Utilities.teleportMyChar(getItemNV3().x, getItemNV3().y);
