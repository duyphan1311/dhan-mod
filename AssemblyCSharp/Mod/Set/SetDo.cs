...
namespace Mod.Set
{
    public class SetDo : IChatable, IActionListener
    {
            ...
            new Thread(() =>
            {
                while (true)
                {
                    ...
                    if (GetItem(itemCaiTrang.fullName, 5, 6))
                        Thread.Sleep(100);
                    if (GetItem(itemGiapLuyenTap.fullName, 6, 6))
                        Thread.Sleep(100);
                    if (DateTime.Now.Subtract(now).TotalMilliseconds < 50)
                        break;
                }
            ...
            for (int i = 0; i < arrItemBody.Length + arrItemBag.Length + arrItemPetBody.Length; i++)
            {
                ...
                if (y1 - GameCanvas.panel.cmy <= GameCanvas.panel.yScroll + GameCanvas.panel.hScroll)
                {
                    if (y1 - GameCanvas.panel.cmy >= GameCanvas.panel.yScroll - GameCanvas.panel.ITEM_HEIGHT)
                    {
                        ...
                        if (item.quantity > 1)
                            mFont.tahoma_7_yellow.drawString(g, $"{item.quantity}", x2 + w2, y2 + itemHeight - mFont.tahoma_7_yellow.getHeight(), 1);
        ...
        public static void LoadData()
        {
            try
            {
                setDos = Utilities.loadRMSString("setdo") == "" ? new List<SetDo>() : JsonMapper.ToObject<List<SetDo>>(Utilities.loadRMSString("setdo"));
            }
        ...
        public static void SaveData()
        {
            try
            {
                Utilities.saveRMSString("setdo", JsonMapper.ToJson(setDos));
            }
            catch (Exception ex) { Debug.LogException(ex); }
        }
