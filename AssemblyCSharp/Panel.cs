...
public class Panel : IActionListener, IChatable
{
	...
	{
		#region mod
		if (!info.isChatServer)
			Messenger.addMessage(info.charInfo.charID, info.charInfo.cName, true, Res.split(info.s, "|", 0)[2]);
		else
	...
	private void updateKeyScrollView()
	{
		...
		if (GameCanvas.isPointerDown)
		{
			justRelease = false;
			#region mod
			if (type == 3 && GameCanvas.isPointer(wScroll - 26, yScroll, 28, hScroll))
			{
				OnScreen.isAutoChange = true;
			}
			#endregion
	...
	public void moveCamera()
	{
		...
		#region mod
		if (OnScreen.isAutoChange && (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0))
		{
			if (Utilities.isChenKhu && OnScreen.zoneInsert == selected)
			{
				Utilities.isChenKhu = false;
				GameCanvas.panel.EmulateSetTypePanel(0);
			}
			else
			{
				Utilities.changeZone(selected);
				GameCanvas.panel.EmulateSetTypePanel(0);
			}
			isChangeZone = false;
			OnScreen.isAutoChange = false;
			selected = -1;
		}
		else if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
		#endregion
		{
			isShow = false;
			cleanCombine();
	...
	private void paintTools(mGraphics g)
	{
		...
		for (int i = 0; i < strTool.Length; i++)
		{
			...
			#region mod
			if (Messenger.hasNewMsg() && GameCanvas.gameTick % 20 > 10)
			{
				if (strTool[i].Equals(mResources.account))
					g.drawImage(imgNew, num3 - 15, num2 + 10, 3);
			}
			#endregion
	...
	private void paintZone(mGraphics g)
	{
		...
		for (int i = 0; i < pts.Length; i++)
		{
			...
			#region mod
			g.setColor((i != selected) || OnScreen.isAutoChange ? 15196114 : 16383818);
			g.fillRect(num, num2, num3 - num5 + 4, h);
			#endregion
			...
			if (zones[i] != -1)
			{
				...
				#region mod
				string stts = GameScr.gI().numPlayer[i] + "/" + GameScr.gI().maxPlayer[i];
				if (GameScr.gI().numPlayer[i] >= GameScr.gI().maxPlayer[i] && !Utilities.isChenKhu)
				{
					mFont.tahoma_7_green2.drawString(g, "[Chọn để auto vào khu]", num + 8 + mFont.tahoma_7_green2.getWidth(stts), num2 + 6, 0);
				}
				if (Utilities.isChenKhu && i == OnScreen.zoneInsert)
				{
					mFont.tahoma_7_green2.drawString(g, "[Đang auto vào khu...]", num + 8 + mFont.tahoma_7_green2.getWidth(stts), num2 + 6, 0);
				}
				mFont.tahoma_7_green2.drawString(g, GameScr.gI().numPlayer[i] + "/" + GameScr.gI().maxPlayer[i], num + 5, num2 + 6, 0);
				OnScreen.paintAutoZone(g, i, num3 + 10, y, num5 - 6, h);
				#endregion
			}
	...
	private void paintBox(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item.quantity > 1)
			{
				mFont.tahoma_7_yellow.drawString(g, $"{item.quantity}", num5 + num7 - 15, num6 + 6, 0);
			}
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (item != null)
			{
				...
				if (AutoUpgrade.listUpgrade.Any(x => x.id == num2 && x.type == item.template.type && x.name == item.template.name))
				{
					g.setColor(color2[0]);
					g.fillRect(num3, num4, num5, h);
					g.setColor(color2[0]);
			...
			if (item.quantity > 1)
			{
				mFont.tahoma_7_yellow.drawString(g, item.quantity.ToString(), num6 + num8, num7 + num9 - mFont.tahoma_7_yellow.getHeight(), 1);
			}
	...
	private void paintTab(mGraphics g)
	{
		...
		for (int i = 0; i < currentTabName.Length; i++)
		{
			...
			#region mod
			if (type == 0 && Messenger.hasNewMsg() && GameCanvas.gameTick % 4 == 0)
			{
				g.drawImage(ItemMap.imageFlare, startTabPos + (currentTabName.Length - 1) * TAB_W + TAB_W / 2, 77, mGraphics.BOTTOM | mGraphics.HCENTER);
			}
			#endregion
		}
		g.setColor(13524492);
	...
	private void paintToolInfo(mGraphics g)
	{
		try
		{
			if (ServerListScreen.ipSelect > ServerListScreen.nameServer.Length - 1)
				ServerListScreen.ipSelect = Utilities.server;
			mFont.tahoma_7b_white.drawString(g, "By Duy Phan v" + GameMidlet.VERSION, 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
			mFont.tahoma_7_yellow.drawString(g, mResources.character + ": " + Char.myCharz().cName, 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
			mFont.tahoma_7_yellow.drawString(g, mResources.account_server + ": " + ServerListScreen.nameServer[Utilities.server], 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
	...
	{
		#region mod
		mFont.tahoma_7b_white.drawString(g, ((GameScr.isNewMember == 1) ? "       " : string.Empty) + $"[{c.charID}] " + c.cName, X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
	...
	public void hide()
	{
		...
		if (type != 4)
		{
			...
			#region mod
			else if (type == 18 || type == 19 || type == 20 || type == 21 /*|| type == CustomPanelMenu.TYPE_CUSTOM_PANEL_MENU*/)
			{
				setTypeMain();
				cmx = (cmtoX = 0);
			}
			else if (type == CustomPanelMenu.TYPE_CUSTOM_PANEL_MENU && Messenger.typePanel != -1)
			{
				if (Messenger.typePanel == 0)
				{
					Messenger.typePanel = -1;
					setTypeAccount();
					cmx = (cmtoX = 0);
				}
				if (Messenger.typePanel == 1)
				{
					Messenger.showMessengerPanel();
					cmx = (cmtoX = 0);
				}
			}
			else if (type == 8)
	...
	private void doSpeacialSkill()
	{
		...
		myVector.addElement(new Command("Mở max", AutoSpecialSkill.gI(), 4, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
		myVector.addElement(new Command("Mở Tùy\nChọn", AutoSpecialSkill.gI(), 5, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
		myVector.addElement(new Command("Mở", AutoSpecialSkill.gI(), 3, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
			...
			if (currItem != null)
			{
				if (currItem.isBuySpec)
				{
					if (currItem.buySpec > 0)
					{
						...
						#region mod
						myVector.addElement(new Command($"Mua nhiều\n{mSystem.numberTostring(currItem.buySpec)}/1", AutoBuy.gI, 3, currItem));
						#endregion
					}
				}
				else if (typeShop == 4)
				...
				else if (typeShop != 2)
				{
					...
					if (currItem.buyCoin > 0)
					{
						...
						myVector.addElement(new Command($"Mua nhiều\n{mSystem.numberTostring(currItem.buyCoin)} {mResources.XU}/1", AutoBuy.gI, 1, currItem));
					}
					if (currItem.buyGold > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyGold) + "\n" + mResources.LUONG, this, 3001, currItem));
						myVector.addElement(new Command($"Mua nhiều\n{mSystem.numberTostring(currItem.buyGold)} {mResources.LUONG}/1", AutoBuy.gI, 2, currItem));
					}
					#endregion
				}
				else
			...
			if (!GetInventorySelect_isbody(selected, newSelected, Char.myCharz().arrItemBody))
			{
				...
				if (item != null)
				{
					...
					else if (item.isTypeBody())
					{
						...
						#region mod
						bool HasItemInList = AutoUpgrade.listUpgrade.Any(x => ((selected - Char.myCharz().arrItemBody.Length) == x.id && currItem.template.type == x.type && currItem.template.name == x.name));
						if (HasItemInList)
							myVector.addElement(new Command("Loại khỏi\nList Upgrade", AutoUpgrade.gI, 1, currItem));
						else
					...
					else
					{
						...
						{
							bool HasItemInList = AutoUseItem.listItemUse.Any(x => (currItem.template.id == x.item.template.id && currItem.GetFullInfo() == x.item.GetFullInfo()));
							if (HasItemInList)
								myVector.addElement(new Command("Kết thúc", AutoUseItem.gI, 2, currItem));
							else
								myVector.addElement(new Command("Dùng liên tục", AutoUseItem.gI, 1, currItem));
						}
						if (item.template.id == 457)
						{
							if (AutoSellGold.isBanVang)
								myVector.addElement(new Command("Kết thúc", AutoSellGold.gI, 2, null));
							else
								myVector.addElement(new Command("Auto bán", AutoSellGold.gI, 1, null));
						}
						#endregion
					}
				}
		...
		try
		{
			...
			else if (isViewMember)
			{
				...
				if (currMem != null)
				{
					...
					#region mod
					if (Utilities.canTeleport())
						myVector4.addElement(new Command(mResources.den, this, 950103, currMem.ID));
					if (currMem.ID != Char.myCharz().charID)
						myVector4.addElement(new Command(mResources.CHAT, Messenger.gI, 1, $"{currMem.ID}|{currMem.name}"));
					#endregion
					if (member != null)
					{
			...
			else if (isMessage)
			{
				...
				if (currMess != null)
				{
					if (currMess.type == 0)
					{
						...
						#region mod
						if (Utilities.canTeleport())
							myVector5.addElement(new Command(mResources.den, this, 950103, currMess.playerId));
						if (currMess.playerId != Char.myCharz().charID)
							myVector5.addElement(new Command(mResources.CHAT, Messenger.gI, 1, $"{currMess.playerId}|{currMess.playerName}"));
						#endregion
	...
	public void perform(int idAction, object p)
	{
		...
		{
			int charID = (int)p;
			TeleportMenu.TeleportToPlayer(charID);
		}
		#endregion
		if (idAction == 9999)
		{
		...
		if (idAction == 8004)
		{
			...
			//TeleportChar teleportChar = new TeleportChar(infoItem3.charInfo);
			TeleportMenu.TeleportToPlayer(infoItem3.charInfo.charID);
	...
	public void onChatFromMe(string text, string to)
	{
		...
		else if (chatTField.strChat.Equals(mResources.chat_player))
		{
			...
			{
				#region mod
				Service.gI().chatPlayer(text, infoItem.charInfo.charID, infoItem.charInfo.cName);
				#endregion
			}
		}
	...
	private void paintAccount(mGraphics g)
	{
		...
		for (int i = 0; i < strAccount.Length; i++)
		{
			...
			#region mod
			if (strAccount[i].Equals(mResources.msg) && GameCanvas.gameTick % 20 > 10 && Messenger.hasNewMsg())
				g.drawImage(imgNew, num2 - 15, num + 10, 3);
			#endregion
		}
		paintScrollArrow(g);
	}
