...
public class Main : MonoBehaviour
{
	...
	private void Start()
	{
		...
		new Thread(new ThreadStart(ALogin.Login)).Start();
	...
	private void OnGUI()
	{
		if (count >= 10)
		{
			...
			checkInput();
			#region mod
			AssemblyCSharp.Mod.ModHelper.MainThreadDispatcher.update();
			#endregion
			Session_ME.update();
			Session_ME2.update();
			if (Event.current.type.Equals(EventType.Repaint) && paintCount <= updateCount)
			{
                ...
                #region mod
                mFont.nameFontRed.drawString(g, $"{System.Math.Round(1f / Time.smoothDeltaTime * Time.timeScale, 1):0.#}", 8, 5, 0);
                #endregion
                paintCount++;
				g.reset();
			}
