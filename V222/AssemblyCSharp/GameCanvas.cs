...
public class GameCanvas : IActionListener
{
	...
	public void update()
	{
		...
		{
			PickMobController.isReconnect = false;
		}
		else
        {
			if (PickMobController.isLoginTime > 0)
			{
				PickMobController.isReconnect = true;
				PickMobController.isLoadGame = true;
			}
		}
		if (PickMob.IsAutoLogin && PickMobController.isReconnect && gameTick % 20 == 0)
		{
	...
	public static void connect()
	{
		...
		{
			#region mod
			if (!AssemblyCSharp.Mod.Options.ServerUse.hostDefault.Contains("nrostar") && !AssemblyCSharp.Mod.Options.ServerUse.hostDefault.Contains("nrosun"))
			{
				GameMidlet.IP = AssemblyCSharp.Mod.Options.ServerUse.hostDefault;
				GameMidlet.PORT = AssemblyCSharp.Mod.Options.ServerUse.port;
			}
			#endregion
	...
	{
		#region mod
		if (info.Contains("Không thể chuyển khu vực"))
			return;
		if (info.Contains("Không thể đổi khu vực trong map này"))
			return;
		if (info.Contains("Khu vực đầy"))
			return;
		//PickMob.startOkDlg(info);
		if(info.Contains("Bạn không đủ tiềm năng"))
        {
			AutoNoiTai.gI().isIncreasePotential = false;
			GameScr.info1.addInfo("Tự động cộng tiềm năng tắt", 0);
			return;
		}
		if (info.Contains("Đã tối đa"))
		{
			...
			//PickMobController.IsDapDo = false;
			GameScr.info1.addInfo("Trang bị đã đạt cấp tối đa. Auto đập đồ tắt", 0);
			return;
		}
		if (info.Contains("trang bị có lỗ sao"))
		{
			AutoUpGrade.isupgrade = false;
			AutoUpGrade.IsShowListUpgrade = false;
			PickMobController.IsDapDo = false;
			//PickMobController.IsDapDo = false;
			GameScr.info1.addInfo("Trang bị đã đạt số sao pha lê tối đa. Auto ép sao pha lê tắt", 0);
			return;
		}
		if(currentScreen != loginScr && currentScreen != serverScreen && currentScreen != serverScr && currentScreen != CreateCharScr.instance)
        {
