...
public class InfoMe
{
	...
	public void addInfo(string s, int Type)
	{
		#region mod
		Pk9rXmap.Info(s);
		UglyBoy.AutoCrackBall.infoMe(s);
		if (s.ToLower().Contains("không thể đổi khu vực này")) return;
		if (s.ToLower().Contains("không thể đổi khu vực tại")) return;
		if (s.Contains("Bạn chưa có đệ tử")) return;
		if (s.Contains("Bạn không đủ tiềm năng"))
		{
			UglyBoy.AutoNoiTai.gI().isIncreasePotential = false;
			GameScr.info1.addInfo("Không đủ tiềm năng. Tự động cộng tiềm năng đã tắt", 0);
			return;
		}
		if (s.Contains("trang bị có lỗ sao"))
		{
			UglyBoy.AutoUpGrade.isupgrade = false;
			UglyBoy.AutoUpGrade.IsShowListUpgrade = false;
			AssemblyCSharp.Mod.PickMob.PickMobController.IsDapDo = false;
