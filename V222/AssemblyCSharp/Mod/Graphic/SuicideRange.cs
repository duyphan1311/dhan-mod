...
namespace Mod.Graphic
{
    public class SuicideRange
    {
        ...
        public static void paint(mGraphics g)
        {
            ...
            int y = GameCanvas.h - 200;
            for (int i = 0; i < GameScr.vCharInMap.size(); i++)
            {
                Char c = GameScr.vCharInMap.elementAt(i) as Char;
                if (c.isStandAndCharge/* && ((c.cFlag != 0 && Char.myCharz().cFlag != 0 && (c.cFlag != Char.myCharz().cFlag || (c.cFlag == 8 && Char.myCharz().cFlag == 8))) || Char.myCharz().cTypePk == 5 || c.cTypePk == 5)*/)
                {
                    if (c.cgender == 1)
                    {
                        g.setColor(Color.yellow);
                        ...
                        string str1 = $"{CharExtensions.getNameWithoutClanTag(c)} [{NinjaUtil.getMoneys(c.cMPFull)}] đang laze {c.charEffectTime.timeExplode}s";
                        g.setColor(new Color(0.2f, 0.2f, 0.2f, 0.6f));
                        g.fillRect(12, y + 2, mFont.tahoma_7_red.getWidth(str1) + 5, 9);
                        mFont.tahoma_7_white.drawString(g, str1, 10, y, mFont.LEFT);
                        y += 10;
                        continue;
                    }
                    if (c.cgender == 0)
                    {
                        ...
                        if (GameCanvas.gameTick % 10 >= 5) g.setColor(Color.red);
                        CustomGraphics.drawLine(g, Char.myCharz().cx, Char.myCharz().cy, c.cx, c.cy, 3);
                        string str1 = $"{CharExtensions.getNameWithoutClanTag(c)} [{NinjaUtil.getMoneys(c.cDamFull)}] đang kênh khi {c.charEffectTime.timeExplode}s";
                        g.setColor(new Color(0.2f, 0.2f, 0.2f, 0.6f));
                        g.fillRect(12, y + 2, mFont.tahoma_7_red.getWidth(str1) + 5, 9);
                        mFont.tahoma_7_white.drawString(g, str1, 10, y, mFont.LEFT);
                        y += 10;
                    ...
                    if (Utilities.getDistance(c, Char.myCharz()) <= CharExtensions.getSuicideRange(c) && c.cgender == 2)
                    {
                        g.setColor(Color.yellow);
                        if (GameCanvas.gameTick % 10 >= 5) g.setColor(Color.red);
                        CustomGraphics.drawLine(g, Char.myCharz().cx, Char.myCharz().cy, c.cx, c.cy, 3);
                        string str1 = $"Đang trong tầm bom của {CharExtensions.getNameWithoutClanTag(c)} [{NinjaUtil.getMoneys(c.cHPFull)}] {c.charEffectTime.timeExplode}s";
                        g.setColor(new Color(0.2f, 0.2f, 0.2f, 0.6f));
                        g.fillRect(12, y + 2, mFont.tahoma_7_red.getWidth(str1) + 5, 9);
                        mFont.tahoma_7_white.drawString(g, str1, 10, y, mFont.LEFT);
                        y += 10;
                    ...
                    if (GameCanvas.gameTick % 10 >= 5) g.setColor(Color.red);
                    CustomGraphics.drawLine(g, Char.myCharz().cx, Char.myCharz().cy, c.cx, c.cy, 3);
                    string str = $"{CharExtensions.getNameWithoutClanTag(c)} [{NinjaUtil.getMoneys(c.cHPFull)}] đang bom {c.charEffectTime.timeExplode}s";
                    g.setColor(new Color(0.2f, 0.2f, 0.2f, 0.6f));
                    g.fillRect(12, y + 2, mFont.tahoma_7_red.getWidth(str) + 5, 9);
                    mFont.tahoma_7_white.drawString(g, str, 10, y, mFont.LEFT);
                    y += 10;
                }
            }
        }
