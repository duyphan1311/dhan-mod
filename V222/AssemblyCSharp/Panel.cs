...
public class Panel : IActionListener, IChatable
{
	...
	#region mod
	public const int TYPE_MENU_MOD = 25;
	#endregion
		...
		for (int i = 0; i < currentListLength; i++)
		{
			...
			if (item.itemOption != null)
			{
				...
				if (item.template.type == 0 || item.template.type == 1 || item.template.type == 2 ||
					item.template.type == 3 || item.template.type == 4 || item.template.id == 529 ||
					item.template.id == 530 || item.template.id == 531 || item.template.id == 534 ||
					item.template.id == 535 || item.template.id == 536 || item.template.type == 5
				)
				{
					mFont.bigNumber_red.drawString(g, AutoUpGrade.getMaxStart(item) + " sao", num + 130, num2 + 5, mFont.RIGHT);
				}
				#endregion
			}
	...
	public void updateKeyScrollView()
	{
		...
		if (cmtoX >= wScroll - 10 && cmx >= wScroll - 10 && position == 0)
		{
			...
			if (isChangeZone)
			{
				...
				if (Char.myCharz().cHP > 0 && Char.myCharz().statusMe != 14)
				{
					...
					if (type == 3)
					{
						if (GameScr.gI().numPlayer[selected] >= GameScr.gI().maxPlayer[selected])
						{
							OnScreen.zoneChen = selected;
							GameScr.chenKhu(selected);
						}
						else
						{
		...
		for (int i = 0; i < arrItemBody.Length + arrPetSkill.Length; i++)
		{
			...
			if (item != null)
			{
				...
				if (item.itemOption != null)
				{
					...
					{
						mFont.bigNumber_red.drawString(g, AutoUpGrade.getMaxStart(item) + " sao", num3 + 130, num4 + 10, mFont.RIGHT);
					}
					#endregion
				}
	...
	private void paintZone(mGraphics g)
	{
		...
		for (int i = 0; i < pts.Length; i++)
		{
			...
			if (zones[i] != -1)
			{
				...
				#region mod
				if (GameScr.gI().numPlayer[i] >= GameScr.gI().maxPlayer[i] && !GameScr.isChenKhu)
				{
					mFont.tahoma_7_green2.drawString(g, "[Chọn để auto vào khu]", num + 30, num2 + 6, 0);
				}
				if (GameScr.gI().numPlayer[i] >= GameScr.gI().maxPlayer[i] && GameScr.isChenKhu && i == OnScreen.zoneChen)
				{
					mFont.tahoma_7_green2.drawString(g, "[Đang auto vào khu...]", num + 30, num2 + 6, 0);
				}
	...
	private void paintBox(mGraphics g)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (item.itemOption != null)
			{
				...
				{
					mFont.bigNumber_red.drawString(g, AutoUpGrade.getMaxStart(item) + " sao", num2 + 130, num3 + 5, mFont.RIGHT);
				}
				#endregion
			}
			SmallImage.drawSmallImage(g, item.template.iconID, num5 + num7 / 2 - ((item.quantity > 1) ? 8 : 0), num6 + num8 / 2, 0, 3);
	...
	public void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (item != null)
			{
				...
				{
					if (it.type == item.template.type && it.id == num2)
					{
			...
			if (item.itemOption != null)
			{
				...
				{
					mFont.bigNumber_red.drawString(g, AutoUpGrade.getMaxStart(item) + " sao", num3 + 130, num4 + 5, mFont.RIGHT);
				}
				#endregion
			}
			...
			#region mod
			mFont.tahoma_7b_dark.drawString(g, item.template.id.ToString(), num6, num7 - 2 /*+ num9 - mFont.tahoma_7_greySmall.getHeight()*/, mFont.LEFT);
			#endregion
			if (item.quantity > 1)
			{
	...
	private void paintTab(mGraphics g)
	{
		...
		if (type == 27)
		{
			g.setColor(13524492);
			g.fillRect(X + 1, 78, W - 2, 1);
			mFont.tahoma_7b_dark.drawString(g, TabMenu.MenuMod, xScroll + wScroll / 2, 59, mFont.CENTER);
			return;
		}
		#endregion
		if (type == 22)
	...
	private void paintPetInfo(mGraphics g)
	{
		...
		{
			mFont.tahoma_7_yellow.drawString(g, mResources.vitality + ": " + NinjaUtil.getMoneys(Char.myPetz().cStamina) + " / " + NinjaUtil.getMoneys(Char.myPetz().cMaxStamina), X + 60, 38, mFont.LEFT, mFont.tahoma_7_grey);
			//g.drawImage(GameScr.imgMPLost, X + 100, 41, 0);
			//int num = Char.myPetz().cStamina/ Char.myPetz().cMaxStamina;
	...
	private void paintCharInfo(mGraphics g, Char c)
	{
		...
		mFont.tahoma_7b_white.drawString(g, ((GameScr.isNewMember == 1) ? "       " : string.Empty) + PickMob.idC + " [" + c.charID + "]", X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		//mFont.tahoma_7b_white.drawString(g, ((GameScr.isNewMember == 1) ? "       " : string.Empty) + "Huy bu lon" + " ["+c.charID+"]", X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		...
		if (c.cMaxStamina > 0)
		{
			mFont.tahoma_7_yellow.drawString(g, mResources.vitality + ": " + NinjaUtil.getMoneys(c.cStamina) + " / " + NinjaUtil.getMoneys(c.cMaxStamina), X + 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
	...
	public void hide()
	{
		...
		if (type != 4)
		{
			...
			#region mod
			else if (type == 18 || type == 19 || type == 20 || type == 21 || type == 27)
			{
				setTypeMain();
				cmx = (cmtoX = 0);
			}
	...
	public void update()
	{
        ...
        if (Input.GetKey((KeyCode)120))
		{
			...
			if (AutoUpGrade.isKham2)
			{
				AutoUpGrade.isKham2 = false;
				GameScr.info1.addInfo("Đã Dừng", 0);
			}
			...
			if (UseItemMultiple.isAutoMuaDo)
			{
				UseItemMultiple.isAutoMuaDo = false;
				UseItemMultiple.listItemBuy.Clear();
				GameScr.info1.addInfo("Auto mua đồ đã Dừng", 0);
			}
			if (UseItemMultiple.isUseItemMulti)
			{
				UseItemMultiple.isUseItemMulti = false;
				UseItemMultiple.itemList.Clear();
				GameScr.info1.addInfo("Auto sử dụng item đã Dừng", 0);
			}
			if (PickMobController.IsDapDo)
			{
				PickMobController.IsDapDo = false;
				GameScr.info1.addInfo("Đã Dừng", 0);
			}
			if (PickMobController.isDoiVang)
			{
				PickMobController.isDoiVang = false;
				GameScr.info1.addInfo("Đã Dừng", 0);
			}
			...
			if (AutoNoiTai.gI().isIncreasePotential)
			{
				AutoNoiTai.gI().isIncreasePotential = false;
				GameScr.info1.addInfo("Đã Dừng", 0);
			}
	...
	private void doSpeacialSkill()
	{
		...
		myVector.addElement(new Command("Mở max", AutoNoiTai.gI(), 4, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
		myVector.addElement(new Command("Mở Tùy\nChọn", AutoNoiTai.gI(), 8, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
		myVector.addElement(new Command("Mở", AutoNoiTai.gI(), 3, Char.myCharz().infoSpeacialSkill[currentTabIndex][selected]));
	...
	private void doFireCombine()
	{
		if (currentTabIndex == 0)
		{
			...
			if (selected == vItemCombine.size())
			{
				...
				myVector1.addElement(new Command("Liên Tục", AutoNoiTai.gI(), 5, null));
				myVector1.addElement(new Command("Nhanh", AutoNoiTai.gI(), 6, null));
				myVector1.addElement(new Command("Khảm SPL", AutoNoiTai.gI(), 12, null));
				myVector1.addElement(new Command("Thường", AutoNoiTai.gI(), 10, null));
			...
			if (currItem != null)
			{
				if (currItem.isBuySpec)
				{
					if (currItem.buySpec > 0)
					{
						...
						#region mod
						myVector.addElement(new Command("Mua nhiều", this, 9506, currItem));
                        #endregion
                    }
                }
				else if (typeShop == 4)
				...
				else if (typeShop != 2)
				{
					if (currItem.buyCoin > 0)
                    {
						...
						myVector.addElement(new Command("Mua nhiều", this, 9504, currItem));
					}
					if (currItem.buyGold > 0)
					{
						myVector.addElement(new Command(mResources.buy_with + "\n" + Res.formatNumber2(currItem.buyGold) + "\n" + mResources.LUONG, this, 3001, currItem));
						myVector.addElement(new Command("Mua nhiều", this, 9505, currItem));
						#endregion
					}
				}
                else
			...
			if (selected >= arrItemBody.Length)
			{
				...
				if (item != null)
				{
					...
					else if (item.isTypeBody())
					{
                        ...
                        #region mod
                        if (item.template.type == 32 || (item.template.type >= 0 && item.template.type <= 5))
						{
							myVector.addElement(new Command("Thêm vào\ndanh sách\nnâng cấp", this, 9503, currItem));
						}
                        #endregion
                    }
                    else
					{
                        ...
                        #region mod
                        if(UseItemMultiple.itemList.Count == 0)
                        {
							myVector.addElement(new Command("Dùng liên tục", this, 9501, currItem));
						}
                        else
                        {
							foreach (UseItemMultiple.ItemUseMulti ita in UseItemMultiple.itemList)
							{
								if (currItem.template.id != ita.item.template.id)
								{
									myVector.addElement(new Command("Dùng liên tục", this, 9501, currItem));
								}
								else
								{
									myVector.addElement(new Command("Kết thúc", this, 9502, currItem));
								}
							}
						}
                        #endregion
	...
	private void doFireClanOption()
	{
		...
		else if (isViewMember)
		{
			...
			if (currMem != null)
			{
				...
				#region mod
				myVector4.addElement(new Command("Dịch\nChuyển", this, 30402, currMem));
				#endregion
		...
		else if (isMessage)
		{
			...
			if (currMess != null)
			{
				if (currMess.type == 0)
				{
					...
					#region mod
					myVector5.addElement(new Command("Dịch\nChuyển", this, 8005, currMess.playerId));
					#endregion
		...
		if (selected == 0 || selected == 1 || selected == 2 || selected == 3 || selected == 4 || selected == 5)
		{
			...
			if (selected == 0)
			{
				...
				if (cTiemNang >= 100 * (2 * (cHPGoc + num2) + 1980) / 2)
				{
					...
					#region mod
					AutoNoiTai.potential = "hp";
					myVector2.addElement(new Command("Tăng nhiều", AutoNoiTai.gI(), 11, AutoNoiTai.potential));
					#endregion
					GameCanvas.menu.startAt(myVector2, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
					addSkillDetail2(selected);
				}
			...
			if (selected == 1)
			{
				...
				if (cTiemNang >= 100 * (2 * (cMPGoc + num2) + 1980) / 2)
				{
					...
					#region mod
					AutoNoiTai.potential = "mp";
					myVector4.addElement(new Command("Tăng nhiều", AutoNoiTai.gI(), 11, AutoNoiTai.potential));
					#endregion
					GameCanvas.menu.startAt(myVector4, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
					addSkillDetail2(selected);
				}
			...
			if (selected == 2)
			{
				...
				if (cTiemNang >= 100 * (2 * cDamGoc + 99) / 2 * Char.myCharz().expForOneAdd)
				{
					...
					#region mod
					AutoNoiTai.potential = "sd";
					myVector6.addElement(new Command("Tăng nhiều", AutoNoiTai.gI(), 11, AutoNoiTai.potential));
					#endregion
			...
			{
				if (Char.myCharz().cName.Trim() != "aquafina")
				{
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 30402)
		{
			...
			PickMobController.DCTT(member.ID);
		}
		#endregion
		if (idAction == 9999)
		{
		...
		if (idAction == 8004)
		{
			...
			PickMobController.DCTT(infoItem3.charInfo.charID);
		}
		if (idAction == 8005)
		{
			int id = (int)p;
			PickMobController.DCTT(id);
		}
		#endregion
		if (idAction == 8001)
		{
		...
		{
			Item[] arrItemBody2 = Char.myCharz().arrItemBody;
			int len = selected - arrItemBody2.Length;
			AutoNoiTai.addListUpgrade(this.currItem, len);
		}
		if (idAction == 9501)
		{
			UseItemMultiple.curr = currItem;
            GameCanvas.panel.hide();
            Chat2Text.strTitle = "Nhập số lượng cần dùng và thời gian delay mỗi lần";
			Chat2Text.strTFCode = "Delay (s)";
			Chat2Text.gI().switchToMe();
		}
		if (idAction == 9502)
		{
			foreach (UseItemMultiple.ItemUseMulti ita in UseItemMultiple.itemList)
            {
				if (ita.item.template.id == currItem.template.id)
				{
					UseItemMultiple.itemList.Remove(ita);
					GameScr.info1.addInfo($"Auto dùng item {ita.item.template.name} đã dừng", 0);
					break;
				}
		...
		{
			UseItemMultiple.curr = currItem;
			UseItemMultiple.typeBuy = 0;
			GameCanvas.panel.hide();
			Chat2Text.strTitle = "Nhập số lượng cần mua và thời gian delay";
			Chat2Text.strTFCode = "Delay (ms)";
			Chat2Text.gI().switchToMe();
