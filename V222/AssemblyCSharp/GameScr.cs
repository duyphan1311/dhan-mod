...
public class GameScr : mScreen, IChatable
{
	...
	public static bool xcommand = true;

	public static bool xskill = true;
	...
	private void loadDefaultonScreenSkill()
	{
		...
		for (int i = 0; i < 8 && i < Char.myCharz().vSkillFight.size(); i++)
		{
			Skill skill = (Skill)Char.myCharz().vSkillFight.elementAt(i);
			onScreenSkill[i] = skill;
		}
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						else if (OnScreen.HotKeys()) { }
						else if (PickMob.HotKeys()) { }
						#endregion
					}
	...
	private void updateKeyTouchControl()
	{
        ...
        {
			OnScreen.UpdateKeyTouchControl();
		}
		#endregion
	}
	...
	{
		#region mod
		Mod.CharEffect.Update();
		SuicideRange.update();
        Main.updateInfo();
        if (GameCanvas.gameTick % 20 == 0)
		{
			UseItemMultiple.update();
			PickMobController.isLoginTime = 1;
		}
        if (GameCanvas.gameTick % 20 == 0 && AutoGa.isAutoGaEnabled)
        {
			AutoGa.update();
        }
        if (AutoUpGrade.isEpNR && GameCanvas.gameTick % 20 == 0)
		{
			AutoUpGrade.EpNR();
		}
		if (AutoUpGrade.isNCTB && GameCanvas.gameTick % 20 == 0)
		{
			AutoUpGrade.NCTB();
		}
		if (GameCanvas.gameTick % 20 == 0 && PickMobController.isBanVang)
		{
			PickMobController.banvang();
		}
		if (GameScr.gI().isBagFull() && GameCanvas.gameTick % 20 == 2)
		{
			PickMobController.isFullBag1 = true;
			if (PickMobController.isVutDo) GameScr.info1.addInfo("Vứt đồ on", 0);
		}
		if (PickMobController.isVutDo && PickMobController.isFullBag1 && GameCanvas.gameTick % 20 == 0)
		{
			PickMobController.AutoVutDo();
		}
		if (GameCanvas.gameTick % (3 * (int)Time.timeScale) == 0)
		{
			if (!Pk9rXmap.IsXmapRunning)
			{
				PickMob.Update();
				PickMob.GameScrUpdate();
			}
			if (GameCanvas.gameTick % (40 * (int)Time.timeScale) == 0)
			{
				Time.timeScale = PickMob.speed;
				global::Char.myCharz().cspeed = PickMob.tdc;
			}
			PickMobController.Revive();

		}
		if (GameCanvas.gameTick % 20 == 0 && TileMap.mapID != 21 && TileMap.mapID != 22 && TileMap.mapID != 23)
		{
		...
		#region mod
		if (xskill && GameCanvas.gameTick % (22 * (int)Time.timeScale) == 0)
		{
	...
	public override void paint(mGraphics g)
	{
		...
		#region mod
		//mFont.nameFontRed.drawString(g, OnScreenController.CalculateFrameRate().ToString(), 8, 5, 0);
		OnScreen.HienSKH(g);
		OnScreen.hienItemRaMH(g);
		OnScreen.OnscreenUpdate(g, cmx, cmy);
		#endregion
	}
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		{
			#region mod
			if (GameCanvas.currentScreen == AssemblyCSharp.Mod.Chat2Text.instance || GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu || isPaintPopup() || GameCanvas.panel.isShow || Char.myCharz().taskMaint.taskId == 0 || ChatTextField.gI().isShow || GameCanvas.currentScreen == MoneyCharge.instance)
			{
				return;
			}
            #endregion
            ...
            if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num4; i++)
				{
					...
					#region mod
					float cds = skill.coolDown - mSystem.currentTimeMillis() + skill.lastTimeUseThisSkill;
					mFont.tahoma_7b_red.drawString(g, (cds > 0) ? string.Concat(string.Format("{0:0.0}", (float)(cds / 1000))) : string.Empty, xSkill + GameScr.xS[i] + 14, GameScr.yS[i] + 7, mFont.CENTER, mFont.tahoma_7);
					#endregion
				}
			}
			paintGamePad(g);
	...
	public static void chenKhu(int khu)
	{
		int oldMap = TileMap.mapID;
		new Thread(delegate ()
		{
			while (TileMap.zoneID != khu && oldMap == TileMap.mapID)
			{
				isChenKhu = true;
				if (GameScr.gI().numPlayer[khu] < GameScr.gI().maxPlayer[khu])
				{
	...
	public static void capsule()
	{
		...
		{
			sbyte capsu = 0;
			while ((int)capsu < global::Char.myCharz().arrItemBag.Length)
			{
				if (global::Char.myCharz().arrItemBag[(int)capsu] == null)
				{
					capsu++;
					continue;
				}
				if (global::Char.myCharz().arrItemBag[(int)capsu].template.id == 194)
				{
					Service.gI().useItem(0, 1, capsu, -1);
					break;
				}
				if (global::Char.myCharz().arrItemBag[(int)capsu].template.id == 193)
				{
					Service.gI().useItem(0, 1, capsu, -1);
					break;
				}
	...
	public void onChatFromMe(string text, string to)
	{
		Res.outz("CHAT");
		#region mod
		if (ChatTextField.gI().strChat == AutoNoiTai.epNRCaption)
		{
			int num5 = -1;
			try
			{
				num5 = int.Parse(ChatTextField.gI().tfChat.getText());
			}
			catch (Exception)
			{
			...
			if (num5 != -1 && num5 >= 1 && num5 <= 1000)
			{
				AutoUpGrade.SoLuongEp = num5;
				ChatTextField.gI().isShow = false;
				ChatTextField.gI().tfChat.setIputType(TField.INPUT_TYPE_ANY);
				new Thread(new ThreadStart(AutoUpGrade.GotoNPC21)).Start();
			...
			{
				GameCanvas.startOKDlg(AutoNoiTai.epNRCaption);
			}
			return;
		}
		#endregion
