...
using UnityEngine;
using static Mod.Auto.AutoUpgrade;
...
public class Panel : IActionListener, IChatable
{
	...
	private void paintInventory(mGraphics g)
	{
		...
		for (int i = 0; i < arrItemBody.Length + arrItemBag.Length; i++)
		{
			...
			if (item != null)
			{
				...
				#region mod
				if (listUpgrade.Any(x => x.id == num2 && x.type == item.template.type && x.name == item.template.name))
				{
					g.setColor(color2[0]);
					g.fillRect(num3, num4, num5, h);
					g.setColor(color2[0]);
	...
	public void perform(int idAction, object p)
	{
		...
		{
			int charID = (int)p;
			//TeleportChar teleportChar = new TeleportChar(charID);
