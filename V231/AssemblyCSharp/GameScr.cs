...
public class GameScr : mScreen, IChatable
{
	...
	public void playerMenu(Char c)
	{
		...
		if (Char.myCharz().taskMaint != null && Char.myCharz().taskMaint.taskId > 1)
		{
            ...
            #region mod
            vPlayerMenu.addElement(new Command("Chat", 131120, Char.myCharz().charFocus));
            #endregion
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
                        ...
                        {
                            ChatTextField.gI().startChat(47, this, string.Empty);
                        }
                        else
                        {
	...
	public override void update()
	{
		...
		{
			//WriteLog.write("log_onUpdateGameScr.txt", e.ToString(), "GameEvents");
		}
        #endregion
	...
	public override void paint(mGraphics g)
	{
		...
		if (flag && !isRongThanXuatHien)
		{
			...
			if (TileMap.tileID != 28)
			{
				for (int i = 0; i < num2; i++)
				{
                    ...
                    #region mod
                    if (num3 + mGraphics.getImageWidth(TileMap.imgLight) >= cmx 
						&& num3 <= cmx + GameCanvas.w && num4 + mGraphics.getImageHeight(TileMap.imgLight) >= cmy 
						&& num4 <= cmy + GameCanvas.h && ModMenuMain.getStatusInt("levelreducegraphics") <= 1)
					#endregion
                    {
		...
		if (!isPaintOther)
		{
            ...
            #region mod
            if ((isNewClanMessage || Messenger.hasNewMsg()) && !GameCanvas.panel.isShow && GameCanvas.gameTick % 4 == 0)
			{
	...
	private void paintSelectedSkill(mGraphics g)
	{
		...
		{
            #region mod
            if (GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null 
				|| GameCanvas.menu.showMenu || isPaintPopup() || GameCanvas.panel.isShow 
				|| Char.myCharz().taskMaint.taskId == 0 || ChatTextField.gI().isShow 
				|| GameCanvas.currentScreen == MoneyCharge.instance
				|| GameCanvas.currentScreen == Chat2TextField.instance)
			{
				return;
			}
            #endregion
	...
	public void onChatFromMe(string text, string to)
	{
		...
		if (to.Equals(mResources.chat_player))
		{
			...
			{
                #region mod
                Service.gI().chatPlayer(text, info2.playerID, info2.info.info.charInfo.cName);
                #endregion
            }
        }
	...
	public void actionPerform(int idAction, object p)
	{
        ...
        if (idAction == 131120)
        {
			Messenger.newChar = (Char)p;
			Messenger.newID = Messenger.newChar.charID;
            Messenger.newName = Messenger.newChar.cName;
			ChatTextField.gI().strChat = string.Empty;
			ChatTextField.gI().tfChat.name = "Nội dung";
			ChatTextField.gI().startChat2(Messenger.gI, $"Chat với {Messenger.newName}");
        }
        #endregion
        switch (idAction)
		{
	...
	private static void setTouchBtn()
	{
		if (isAnalog != 0)
		{
			...
			if (gamePad.isLargeGamePad)
			{
                ...
                Skill[] array = ((!GameCanvas.isTouch) ? keySkill : onScreenSkill);
                xSkill = GameCanvas.w - array.Length * wSkill - 30;
                #endregion
                wSkill = 35;
				xHP = xF - 45;
			}
