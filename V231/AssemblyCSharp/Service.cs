...
public class Service
{
				...
				&& !Utilities.HasOption(Char.myCharz().arrItemBody[5], 33)))
            {
                ...
                for (int i = 0; i < Char.myCharz().arrItemBag.Length; i++)
                {
                    ...
                    if (item != null && Utilities.HasOption(item, 33))
                    {
                        ...
                        while (!Utilities.HasOption(Char.myCharz().arrItemBody[5], 33));
                        break;
                    }
                }
            }
    ...
    {
        #region mod
        if (!FakeIPhoneClient.HasClientType()) Rms.saveRMSInt("clienttype", FakeIPhoneClient.isEnabled ? 7 : 4);
        #endregion
    ...
    #region mod
    public void chatPlayer(string text, int id, string name)
	{
		if(Char.myCharz().charID == id)
		{
			GameScr.info1.addInfo("Không thể tự nhắn cho chính mình!", 0);
			return;
		}
		if (mSystem.currentTimeMillis() - Messenger.lastTimeChat < 10000L)
		{
			GameScr.info1.addInfo($"Vui lòng thử lại sau {(int)((10000L - (mSystem.currentTimeMillis() - Messenger.lastTimeChat)) / 1000)}s", 0);
			return;
        }
        Messenger.lastTimeChat = mSystem.currentTimeMillis();
		...
		try
        {
			...
			message.writer().writeUTF(text);
			session.sendMessage(message);
            Messenger.addMessage(id, name, false, text);
            if (Messenger.typePanel == 1)
            {
                Messenger.setConversation(GameCanvas.panel);
                Utilities.EmulateSetTypePanel(GameCanvas.panel, 0);
            }
            else
            {
                Messenger.conversation = Messenger.conversationList.Find(x => x.id == id);
