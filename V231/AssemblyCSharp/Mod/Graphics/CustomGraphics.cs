...
namespace Mod.Graphics
{
    public static class CustomGraphics
    {
        ...
        static int[] array2 = new int[] { 0, 0, 0, 0, 600841, 3346944, 4587571, 6684682 };
        ...
        static int[] colorUpgrade = new int[18]
        {
        ...
        public static void PaintItemEffectInPanel(mGraphics g, int x, int y, int w, int h, Item item)
        {
            ...
            //    //}
            //    //else 
            //    if (iOption.optionTemplate.id <= 36 && iOption.optionTemplate.id >= 34)    //tinh ấn/nguyệt ấn/nhật ấn
            //        itemOption = iOption;
            ...
            //if (param <= 0)
            //    return;
            //if (param >= 4 && param <= 7 && (/*id > 36 || id < 34 || */id == 72))
            //{
            ...
            //}
            #endregion
            paintBGItem(g, x, y, w, h, item);
            paintEffectItem(g, x, y, w, h, item);
        }

        private static void paintBGItem(mGraphics g, int x, int y, int w, int h, Item item)
        {
            if (item.itemOption == null)
                return;
            for (int i = 0; i < item.itemOption.Length; i++)
        ...
        private static void paintEffectItem(mGraphics g, int x, int y, int w, int h, Item item)
        {
            if (item.itemOption == null)
                return;
            for (int i = 0; i < item.itemOption.Length; i++)
            {
                ...
                if (iOption.optionTemplate == null)
                    continue;
                int param = iOption.param;
                if (iOption.optionTemplate.id == 107) //đồ sao
                {
                    for (int j = 0; j < size.Length; j++)
                    {
                        int fx = x - w / 2 + 1 + upgradeEffectX(GameCanvas.gameTick - j * 4, w - 2);
