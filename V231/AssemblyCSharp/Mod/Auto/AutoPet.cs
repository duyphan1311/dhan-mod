...
namespace Mod.Auto
{
    public class AutoPet
    {
        ...
        public static int mpBuff = 5;
        public static int hpBuff = 5;

        private static long lastTimeChangeZone = 0;

        public static void update()
        {
            if (mode == AutoPetMode.Disabled) return;
            if (mSystem.currentTimeMillis() - lastTimeChangeZone > 1200000L)
            {
                lastTimeChangeZone = mSystem.currentTimeMillis();
            ...
            if (mSystem.currentTimeMillis() - delayCheckPet < 1000) return;
            if (GameCanvas.gameTick % (30 * Time.timeScale) != 0 || XmapController.gI.IsActing) return;
            AutoPick();
            if ((Char.myPetz().cStamina < 5 || Char.myPetz().cMP < Char.myPetz().cMPFull * mpBuff / 100 || Char.myPetz().cHP < Char.myPetz().cHPFull * hpBuff / 100) 
                && GameScr.hpPotion > 0 && !(Char.myPetz().cHP <= 0 || Char.myPetz().isDie)) 
                GameScr.gI().doUseHP();
