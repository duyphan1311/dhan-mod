...
namespace Mod
{
    public class Boss
    {
        ...
        public static readonly List<string> strBossHasBeenKilled = new List<string>()
        {
        ...
        private void GetMapId(string name, string map)
        {
            ...
            if (hasSpecialChar) mapId = idmap;
        }

        static bool hasSpecialChar = false;
        static int idmap = -1;

        public static void AddBoss(string chatVip)
        {
            if (chatVip.ToLower().Contains("nhân bản")) return;
            if (strBossHasBeenKilled.Any(chatVip.Contains))
            {
                if (chatVip.Contains("Người chơi ")) chatVip = chatVip.Replace("Người chơi ", "");
                ...
                try
                {
                    boss = listBosses.Last(b =>
                    {
                        ...
                        //        return false;
                        //}
                        return b.name == array[1].Trim() && string.IsNullOrEmpty(b.killer);
                    });
                }
                ...
                if (boss == null)
                {
                    if (!string.IsNullOrEmpty(array[2]))
                        boss = new Boss(array[1].Trim(), array[2].Trim());
                    else
                        boss = new Boss(array[1].Trim(), "");
                    listBosses.Add(boss);
                }
                boss.isDied = true;
                boss.killer = array[0];
            ...
            else if (chatVip.StartsWith(strBossAppeared[0]))
            {
                ...
                if (array[2].Contains('('))
                {
                    int len = array[2].IndexOf(')') - (array[2].IndexOf('(') + 1);
                    idmap = int.Parse(array[2].Substring(array[2].IndexOf('(') + 1, len));
                    hasSpecialChar = true;
                    Array.Resize(ref array, array.Length + 1);
                    int flag = 0;
                    for(int i = array[2].IndexOf('(') - 2; i >= 0; i--)
                    {
                        if (int.TryParse(array[2][i].ToString(), out int res))
                            flag++;
                        if (!int.TryParse(array[2][i].ToString(), out int res1))
                        {
                            array[3] = array[2].Substring(i + 1, flag).Trim();
                            array[2] = array[2].Remove(i + 1).Trim();
                            break;
                        }
                    }
                }
                ...
                try
                {
                    int mapId = GetMapID(array[2].Trim());
                    boss = listBosses.Last(b =>
                    {
                        if (new int[] { 79, 82, 83 }.Contains(mapId))
                        {
                            if (Regex.IsMatch(array[1].Trim(), "(Tiểu đội trưởng|(Captain|Kapten) Ginyu|Số [1-4]|Jeice|Burter|Recoome|Guldo)"))
                                return false;
                        }
                        return string.IsNullOrEmpty(b.map) && b.name == array[1].Trim();
                    });
                }
                catch (InvalidOperationException) { }
                if (boss == null)
                ...
                {
                    boss.map = array[2].Trim();
                    boss.GetMapId(boss.name, boss.map);
                }
                if (array.Length == 4)
                    boss.zoneId = int.Parse(array[3].Trim());
        ...
        static void PaintListBosses(mGraphics g)
        {
            ...
            for (int i = start - offset; i < listBosses.Count - offset; i++)
            {
                ...
                int length = Utilities.getWidth(styles[i - start + offset], $"{i + 1}. {boss.ToString(true)}");
        ...
        public static void UpdateTouch()
        {
            ...
            if (GameCanvas.isPointerHoldIn(collapseButtonX, collapseButtonY, 6, 9) || GameCanvas.isMouseFocus(GameCanvas.w - (x + offsetX) - titleWidth + scrollBarWidth, y - distanceBetweenLines, titleWidth, 8))
            {
                ...
                if (GameCanvas.isPointerClick)
                {
                    ...
                    Utilities.saveRMSBool("isCollapsedBoss", isCollapsed);
                }
        ...
        public static void Update()
        {
            ...
            {
                Boss boss = listBosses[i];
                int mapid = boss.mapId;
                //if (TileMap.mapNames[78].Equals("Núi khỉ đỏ"))
                //{
                //    if (boss.mapId >= 78) mapid = boss.mapId - 1;
                //    if (boss.mapId >= 94) mapid = boss.mapId - 2;
                //    if (boss.mapId >= 99) mapid = boss.mapId - 3;
                //}
                if (mapid == TileMap.mapID && !Char.isLoadingMap)
                {
                    int j = 0;
                    for (; j < GameScr.vCharInMap.size(); j++)
                    {
                        Char ch = GameScr.vCharInMap.elementAt(j) as Char;
                        if (ch.cName.Trim() == boss.name)
                        {
                            if (boss.zoneId == -1)
        ...
        public static void LoadData()
        {
            try
            {
                isCollapsed = Utilities.loadRMSBool("isCollapsedBoss");
            }
